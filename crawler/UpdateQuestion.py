...
class UpdateOneQuestion(threading.Thread):
	def __init__(self, useProxy, queue):
		self.queue = queue
...
class UpdateQuestions:
	...
	def run(self):
		...
		time_now = int(time.time())
		before_last_visit_time = time_now - 12 * 60 * 60  # 12小时未访问
		after_add_time = time_now - 24 * 3600 * 14  # 14天内添加的

		results = select(HOT_QUESTION_COLL, {"updateTime": {"$lt": before_last_visit_time}, "addTime": {"$gt": after_add_time},
						  "answer": {"$lt": 8}, "topAnswerNum": {"$lt": 50}}, sort=('updateTime', ASCENDING))
		...
		for i in range(thread_amount):
			threads.append(UpdateOneQuestion(self.useProxy, queue))
